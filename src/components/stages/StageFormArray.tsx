...
import { StageColumns } from "@/components/stages/StageColumns";
import { StageMultiCreateDeleteDialog } from "@/components/stages/StageMultiCreateDeleteDialog";
...
interface StageFormArrayProps {
  formik: FormikProps<{ Stages: StageFormikShape[] }>;
}

const StageFormArray: React.FC<StageFormArrayProps> = ({ formik }) => {
  //URL States
  const { router, query, pathname } = useURL<StageSearchParams>();
  ...
  const { setRecordsToDelete } = useStageDeleteDialog();
  ...
  const {
    ...
    isFetching,
    fetchNextPage,
  } = useInfiniteQuery<GetStagesResponse>(["stages"], { enabled: false });
  ...
  const getCurrentData = (page: number) => {
    return [
      ...stageData!.pages[page - 1].rows.map((item, index) => ({
        ...item,
        index,
        touched: false,
      })),
      {
        ...DEFAULT_STAGE,
        index: stageData!.pages[page - 1].rows.length,
      },
    ];
  };
  ...
  const addRow = () => {
    formik.setFieldValue(`Stages`, [
      ...formik.values.Stages.map((item) => ({ ...item })),
      { ...DEFAULT_STAGE },
    ]);
    setWillFocus(true);
  };
  ...
  const setTouchedRows = (idx: number) => {
    formik.setFieldValue(`Stages[${idx}].touched`, true);
  };

  const deleteRow = (idx: number) => {
    const id = formik.values.Stages[idx].id;

    if (id) {
      setRecordsToDelete([id.toString()]);
    } else {
      formik.setFieldValue(`Stages`, [
        ...formik.values.Stages.slice(0, idx),
        ...formik.values.Stages.slice(idx + 1),
      ]);
      formik.setErrors({});
      resetRowSelection();
    }
  ...
  const deleteSelectedRows = () => {
    ...
    const deletedIDs = formik.values.Stages.filter((_, idx) =>
    ...
    if (deletedIDs.length > 0) {
      ...
      formik.setFieldValue(
        `Stages`,
        removeItemsByIndexes(formik.values.Stages, indexes)
      );
      formik.setErrors({});
      resetRowSelection();
    }
  ...
  const toggleSelectAllRow = () => {
    if (Object.keys(rowSelection).length === formik.values.Stages.length) {
      resetRowSelection();
    } else {
      setRowSelectionToAll(formik.values.Stages.length);
    }
  };
  ...
  const stageTable = useReactTable<StageFormikShape>({
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-4">
          <div className="text-sm">
            {stageTable.getFilteredSelectedRowModel().rows.length} of{" "}
            {stageTable.getFilteredRowModel().rows.length} row(s) selected.
          </div>
          {hasSelected && (
            <Button
              type="button"
      ...
      <StageMultiCreateDeleteDialog />
    </Form>
  );
};

export default StageFormArray;
