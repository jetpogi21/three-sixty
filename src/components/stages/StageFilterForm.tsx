...
import { DEFAULT_FILTERS,CONTROL_OPTIONS, } from "@/utils/constants/StageConstants";
...
const StageFilterForm: React.FC = () => {
  ...
  const initialValues: StageFormikFilter = getFilterValueFromURL(
    query,
    defaultFilters
  );
  ...
  //Zustand stores
  const [setPage, setLastPage, setFetchCount, resetRowSelection, setCursor] = useStageStore(
    (state) => [
      state.setPage,
      state.setLastPage,
      state.setFetchCount,
  ...
  const handleFormikSubmit = (
    values: Partial<StageFormikFilter>,
    formik: FormikHelpers<StageFormikFilter>
  ) => {
    const params = {
      ...query,
      ...(getParamsObject(values, defaultFilters) as Partial<StageSearchParams>),
    };
  ...
  const renderFormik = (formik: FormikProps<StageFormikFilter>) => {
...
const filtered = isEqual(defaultFilters, formik.values);
    ...
    const handleClearClick: MouseEventHandler = (e) => {
      e.preventDefault();
      formik.setValues(defaultFilters as StageFormikFilter);
formik.submitForm();
    };
...
export default StageFilterForm;
