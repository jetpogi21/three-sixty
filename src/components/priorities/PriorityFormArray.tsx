...
import { PriorityColumns } from "@/components/priorities/PriorityColumns";
import { PriorityMultiCreateDeleteDialog } from "@/components/priorities/PriorityMultiCreateDeleteDialog";
...
interface PriorityFormArrayProps {
  formik: FormikProps<{ Priorities: PriorityFormikShape[] }>;
}

const PriorityFormArray: React.FC<PriorityFormArrayProps> = ({ formik }) => {
  //URL States
  ...
  const { setRecordsToDelete } = usePriorityDeleteDialog();
  ...
  const {
    ...
    isFetching,
    fetchNextPage,
  } = useInfiniteQuery<GetPrioritiesResponse>(["priorities"], { enabled: false });
  ...
  const dataRowCount = priorityData
    ? priorityData.pages
        .slice(0, page)
  ...
  const getCurrentData = (page: number) => {
    return [
      ...priorityData!.pages[page - 1].rows.map((item, index) => ({
        ...item,
        index,
        touched: false,
      })),
      {
        ...DEFAULT_PRIORITY,
        index: priorityData!.pages[page - 1].rows.length,
      },
    ];
  };
  ...
  const addRow = () => {
    formik.setFieldValue(`Priorities`, [
      ...formik.values.Priorities.map((item) => ({ ...item })),
      { ...DEFAULT_PRIORITY },
    ]);
    setWillFocus(true);
  };
  ...
  const setTouchedRows = (idx: number) => {
    formik.setFieldValue(`Priorities[${idx}].touched`, true);
  };

  const deleteRow = (idx: number) => {
    const id = formik.values.Priorities[idx].id;

    if (id) {
      setRecordsToDelete([id.toString()]);
    } else {
      formik.setFieldValue(`Priorities`, [
        ...formik.values.Priorities.slice(0, idx),
        ...formik.values.Priorities.slice(idx + 1),
      ]);
      formik.setErrors({});
      resetRowSelection();
    }
  ...
  const deleteSelectedRows = () => {
    ...
    const deletedIDs = formik.values.Priorities.filter((_, idx) =>
    ...
    if (deletedIDs.length > 0) {
      ...
      formik.setFieldValue(
        `Priorities`,
        removeItemsByIndexes(formik.values.Priorities, indexes)
      );
      formik.setErrors({});
      resetRowSelection();
    }
  ...
  const toggleSelectAllRow = () => {
    if (Object.keys(rowSelection).length === formik.values.Priorities.length) {
      resetRowSelection();
    } else {
      setRowSelectionToAll(formik.values.Priorities.length);
    }
  };
  ...
  const priorityTable = useReactTable<PriorityFormikShape>({
    data: formik.values.Priorities,
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-4">
          <div className="text-sm">
            {priorityTable.getFilteredSelectedRowModel().rows.length} of{" "}
            {priorityTable.getFilteredRowModel().rows.length} row(s) selected.
          </div>
          {hasSelected && (
            <Button
              type="button"
        ...
        <div className="border rounded-md">
          <Table>
            <TableHeader>
              {priorityTable.getHeaderGroups().map((headerGroup) => (
            ...
            <TableBody>
              {priorityTable.getRowModel().rows?.length ? (
                priorityTable.getRowModel().rows.map((row) => (
                  <TableRow
                    key={row.id}
                ...
                <TableRow>
                  <TableCell
                    colSpan={PriorityColumns.length}
      ...
      <PriorityMultiCreateDeleteDialog />
    </Form>
  );
};

export default PriorityFormArray;
