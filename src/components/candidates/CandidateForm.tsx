...
const CandidateForm: React.FC<CandidateFormProps> = (prop) => {
  ...
  const [originalCandidateStatus, setOriginalCandidateStatus] = useState(
    prop.data
      ? prop.data.CandidateStatus.map((item) => ({
          id: item.id,
  ...
  const initialValues: CandidateFormFormikInitialValues = {
    ...
    CandidateStatus: [], //Generated by GetRelatedModelEmptyArraySimpleOnly - GetRelatedModelEmptyArraySimpleOnly
  };
  ...
  if (candidate) {
    ...
    initialValues.CandidateStatus = candidate.CandidateStatus.map(
      (item, index) => item.statusID.toString()
    );
  }
  ...
  const handleFormikSubmit = (
    ...
    const deletedCandidateStatus =
      originalCandidateStatus
        .filter(
          (item) =>
            !convertArrayItemsToStrings(values.CandidateStatus).includes(
              item.statusID.toString()
            )
        )
        .map((item) => item.id.toString()) || [];

    const newStatus = convertArrayItemsToStrings(values.CandidateStatus).filter(
      (item) =>
        !originalCandidateStatus
          .map((candidateStatus) => candidateStatus.statusID.toString())
          .includes(item)
    );
      ...
      candidateMutation
        ...
        .then((data) => {
          if (addNew) {
            ...
            const newOriginalCandidateStatus = [
              ...originalCandidateStatus,
              ...data.CandidateStatus.map((item) => ({
                id: item.id,
                statusID: item.statusID,
              })),
            ].filter(
              (item) => !deletedCandidateStatus.includes(item.id.toString())
            );

            setOriginalCandidateStatus(newOriginalCandidateStatus);
          }
