...
import { DEFAULT_FILTERS,CONTROL_OPTIONS, } from "@/utils/constants/StatusConstants";
...
const StatusFilterForm: React.FC = () => {
  ...
  const initialValues: StatusFormikFilter = getFilterValueFromURL(
    query,
    defaultFilters
  );
  ...
  //Zustand stores
  const [setPage, setLastPage, setFetchCount, resetRowSelection, setCursor] = useStatusStore(
    (state) => [
      state.setPage,
      state.setLastPage,
      state.setFetchCount,
  ...
  const handleFormikSubmit = (
    values: Partial<StatusFormikFilter>,
    formik: FormikHelpers<StatusFormikFilter>
  ) => {
    const params = {
      ...query,
      ...(getParamsObject(values, defaultFilters) as Partial<StatusSearchParams>),
    };
  ...
  const renderFormik = (formik: FormikProps<StatusFormikFilter>) => {
...
const filtered = isEqual(defaultFilters, formik.values);
    ...
    const handleClearClick: MouseEventHandler = (e) => {
      e.preventDefault();
      formik.setValues(defaultFilters as StatusFormikFilter);
formik.submitForm();
    };
