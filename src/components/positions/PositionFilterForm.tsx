...
import { DEFAULT_FILTERS,CONTROL_OPTIONS, } from "@/utils/constants/PositionConstants";
...
const PositionFilterForm: React.FC = () => {
  ...
  const initialValues: PositionFormikFilter = getFilterValueFromURL(
    query,
    defaultFilters
  );
  ...
  //Zustand stores
  const [setPage, setLastPage, setFetchCount, resetRowSelection, setCursor] = usePositionStore(
    (state) => [
      state.setPage,
      state.setLastPage,
      state.setFetchCount,
  ...
  const handleFormikSubmit = (
    values: Partial<PositionFormikFilter>,
    formik: FormikHelpers<PositionFormikFilter>
  ) => {
    const params = {
      ...query,
      ...(getParamsObject(values, defaultFilters) as Partial<PositionSearchParams>),
    };
  ...
  const renderFormik = (formik: FormikProps<PositionFormikFilter>) => {
...
const filtered = isEqual(defaultFilters, formik.values);
    ...
    const handleClearClick: MouseEventHandler = (e) => {
      e.preventDefault();
      formik.setValues(defaultFilters as PositionFormikFilter);
formik.submitForm();
    };
