...
import { PositionMultiCreateDeleteDialog } from "@/components/positions/PositionMultiCreateDeleteDialog";
...
interface PositionFormArrayProps {
  formik: FormikProps<{ Positions: PositionFormikShape[] }>;
}

const PositionFormArray: React.FC<PositionFormArrayProps> = ({ formik }) => {
  //URL States
  const { router, query, pathname } = useURL<PositionSearchParams>();
  ...
  const { setRecordsToDelete } = usePositionDeleteDialog();
  ...
  const {
    ...
    isFetching,
    fetchNextPage,
  } = useInfiniteQuery<GetPositionsResponse>(["positions"], { enabled: false });
  ...
  const dataRowCount = positionData
    ? positionData.pages
        .slice(0, page)
  ...
  const getCurrentData = (page: number) => {
    return [
      ...positionData!.pages[page - 1].rows.map((item, index) => ({
        ...item,
        index,
        touched: false,
      })),
      {
        ...DEFAULT_POSITION,
        index: positionData!.pages[page - 1].rows.length,
      },
    ];
  };
  ...
  const addRow = () => {
    formik.setFieldValue(`Positions`, [
      ...formik.values.Positions.map((item) => ({ ...item })),
      { ...DEFAULT_POSITION },
    ]);
    setWillFocus(true);
  };
  ...
  const setTouchedRows = (idx: number) => {
    formik.setFieldValue(`Positions[${idx}].touched`, true);
  };

  const deleteRow = (idx: number) => {
    const id = formik.values.Positions[idx].id;

    if (id) {
      setRecordsToDelete([id.toString()]);
    } else {
      formik.setFieldValue(`Positions`, [
        ...formik.values.Positions.slice(0, idx),
        ...formik.values.Positions.slice(idx + 1),
      ]);
      formik.setErrors({});
      resetRowSelection();
    }
  ...
  const deleteSelectedRows = () => {
    ...
    const deletedIDs = formik.values.Positions.filter((_, idx) =>
    ...
    if (deletedIDs.length > 0) {
      ...
      formik.setFieldValue(
        `Positions`,
        removeItemsByIndexes(formik.values.Positions, indexes)
      );
      formik.setErrors({});
      resetRowSelection();
    }
  ...
  const toggleSelectAllRow = () => {
    if (Object.keys(rowSelection).length === formik.values.Positions.length) {
      resetRowSelection();
    } else {
      setRowSelectionToAll(formik.values.Positions.length);
    }
  };
  ...
  const positionTable = useReactTable<PositionFormikShape>({
    data: formik.values.Positions,
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-4">
          <div className="text-sm">
            {positionTable.getFilteredSelectedRowModel().rows.length} of{" "}
            {positionTable.getFilteredRowModel().rows.length} row(s) selected.
          </div>
          {hasSelected && (
            <Button
              type="button"
      ...
      <PositionMultiCreateDeleteDialog />
    </Form>
  );
};
