...
import { DEFAULT_FILTERS,CONTROL_OPTIONS, } from "@/utils/constants/EmployerOfficeConstants";
...
const EmployerOfficeFilterForm: React.FC = () => {
  ...
  const initialValues: EmployerOfficeFormikFilter = getFilterValueFromURL(
    query,
    defaultFilters
  );
  ...
  //Zustand stores
  const [setPage, setLastPage, setFetchCount, resetRowSelection, setCursor] = useEmployerOfficeStore(
    (state) => [
      state.setPage,
      state.setLastPage,
      state.setFetchCount,
  ...
  const handleFormikSubmit = (
    values: Partial<EmployerOfficeFormikFilter>,
    formik: FormikHelpers<EmployerOfficeFormikFilter>
  ) => {
    const params = {
      ...query,
      ...(getParamsObject(values, defaultFilters) as Partial<EmployerOfficeSearchParams>),
    };
  ...
  const renderFormik = (formik: FormikProps<EmployerOfficeFormikFilter>) => {
...
const filtered = isEqual(defaultFilters, formik.values);
    ...
    const handleClearClick: MouseEventHandler = (e) => {
      e.preventDefault();
      formik.setValues(defaultFilters as EmployerOfficeFormikFilter);
formik.submitForm();
    };
...
<FormikControl
  name="q"
  placeholder="Filter Employer Offices..."
  type="Text"
...
export default EmployerOfficeFilterForm;
