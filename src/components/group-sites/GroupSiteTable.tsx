...
import { GroupSiteArraySchema } from "@/schema/GroupSiteSchema";
...
import GroupSiteFormArray from "@/components/group-sites/GroupSiteFormArray";

const GroupSiteTable: React.FC = () => {
  const { query } = useURL<GroupSiteSearchParams>();
...
const q = query["q"] || ""
  ...
  //API Functions
  const getGroupSites = async ({ pageParam = "" }) => {
...
q}, DEFAULT_FILTERS, {
  ...
  const updateGroupSites = async (payload: GroupSiteUpdatePayload) => {
    const { data } = await axiosClient({
      url: "group-sites/multi",
      method: "post",
      data: payload,
...
type MutationData = { recordsCreated?: number; recordsDeleted?: number };
  ...
  // Usage for deleteGroupSiteMutation
  const deleteGroupSiteMutation = useHandleMutation(
    deleteGroupSites,
    (data) => {
      return "Group Site(s) deleted successfully";
    },
    (recordCount, data) => {
  ...
  // Usage for updateGroupSites
  const updateGroupSitesMutation = useHandleMutation(
    updateGroupSites,
    (data) => {
      return "Group Site list updated successfully";
    },
    (recordCount, data) => {
      return (
  ...
  const sliceQueryDataAndRefetch = (idx: number) => {
    queryClient.setQueryData(
      ["groupSites"],
      (data: InfiniteData<GetGroupSitesResponse> | undefined) => {
        return data
          ? {
  ...
  //Client Actions
  const handleSubmit = async (values: GroupSiteFormikInitialValues) => {
    //The reference is the index of the row
    const GroupSitesToBeSubmitted = values.GroupSites.filter((item) => item.touched);

    if (GroupSitesToBeSubmitted.length > 0) {
      const payload: GroupSiteUpdatePayload = {
        GroupSites: GroupSitesToBeSubmitted,
      };

      updateGroupSitesMutation(payload);
    }
  };

  useEffect(() => {
    setMounted(true);
    setMutate(deleteGroupSiteMutation);
  }, []);
...
q]);
  ...
  return (
    <Formik
      initialValues={{
        GroupSites: currentData,
      }}
      enableReinitialize={true}
      onSubmit={handleSubmit}
      validationSchema={GroupSiteArraySchema}
      validateOnChange={false}
    >
      {(formik) => <GroupSiteFormArray formik={formik} />}
    </Formik>
  );
};

export default GroupSiteTable;
