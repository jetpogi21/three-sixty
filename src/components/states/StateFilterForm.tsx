...
import { DEFAULT_FILTERS,CONTROL_OPTIONS, } from "@/utils/constants/StateConstants";
...
const StateFilterForm: React.FC = () => {
  ...
  const initialValues: StateFormikFilter = getFilterValueFromURL(
    query,
    defaultFilters
  );
  ...
  //Zustand stores
  const [setPage, setLastPage, setFetchCount, resetRowSelection, setCursor] = useStateStore(
    (state) => [
      state.setPage,
      state.setLastPage,
      state.setFetchCount,
  ...
  const handleFormikSubmit = (
    values: Partial<StateFormikFilter>,
    formik: FormikHelpers<StateFormikFilter>
  ) => {
    const params = {
      ...query,
      ...(getParamsObject(values, defaultFilters) as Partial<StateSearchParams>),
    };
  ...
  const renderFormik = (formik: FormikProps<StateFormikFilter>) => {
...
const filtered = isEqual(defaultFilters, formik.values);
    ...
    const handleClearClick: MouseEventHandler = (e) => {
      e.preventDefault();
      formik.setValues(defaultFilters as StateFormikFilter);
formik.submitForm();
    };
