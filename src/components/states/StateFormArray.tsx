...
import { StateMultiCreateDeleteDialog } from "@/components/states/StateMultiCreateDeleteDialog";
...
interface StateFormArrayProps {
  formik: FormikProps<{ States: StateFormikShape[] }>;
}

const StateFormArray: React.FC<StateFormArrayProps> = ({ formik }) => {
  //URL States
  const { router, query, pathname } = useURL<StateSearchParams>();
  ...
  const { setRecordsToDelete } = useStateDeleteDialog();
  ...
  const {
    ...
    isFetching,
    fetchNextPage,
  } = useInfiniteQuery<GetStatesResponse>(["states"], { enabled: false });
  ...
  const getCurrentData = (page: number) => {
    return [
      ...stateData!.pages[page - 1].rows.map((item, index) => ({
        ...item,
        index,
        touched: false,
      })),
      {
        ...DEFAULT_STATE,
        index: stateData!.pages[page - 1].rows.length,
      },
    ];
  };
  ...
  const addRow = () => {
    formik.setFieldValue(`States`, [
      ...formik.values.States.map((item) => ({ ...item })),
      { ...DEFAULT_STATE },
    ]);
    setWillFocus(true);
  };
  ...
  const setTouchedRows = (idx: number) => {
    formik.setFieldValue(`States[${idx}].touched`, true);
  };

  const deleteRow = (idx: number) => {
    const id = formik.values.States[idx].id;

    if (id) {
      setRecordsToDelete([id.toString()]);
    } else {
      formik.setFieldValue(`States`, [
        ...formik.values.States.slice(0, idx),
        ...formik.values.States.slice(idx + 1),
      ]);
      formik.setErrors({});
      resetRowSelection();
    }
  ...
  const deleteSelectedRows = () => {
    ...
    const deletedIDs = formik.values.States.filter((_, idx) =>
    ...
    if (deletedIDs.length > 0) {
      ...
      formik.setFieldValue(
        `States`,
        removeItemsByIndexes(formik.values.States, indexes)
      );
      formik.setErrors({});
      resetRowSelection();
    }
  ...
  const toggleSelectAllRow = () => {
    if (Object.keys(rowSelection).length === formik.values.States.length) {
      resetRowSelection();
    } else {
      setRowSelectionToAll(formik.values.States.length);
    }
  };
  ...
  const stateTable = useReactTable<StateFormikShape>({
    data: formik.values.States,
    ...
    >
        <div className="flex items-center gap-4">
          <div className="text-sm">
            {stateTable.getFilteredSelectedRowModel().rows.length} of{" "}
            {stateTable.getFilteredRowModel().rows.length} row(s) selected.
          </div>
          {hasSelected && (
            <Button
              type="button"
      ...
      <StateMultiCreateDeleteDialog />
    </Form>
  );
};

export default StateFormArray;
