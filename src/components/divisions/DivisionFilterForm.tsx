...
import { DEFAULT_FILTERS,CONTROL_OPTIONS, } from "@/utils/constants/DivisionConstants";
...
const DivisionFilterForm: React.FC = () => {
  ...
  const initialValues: DivisionFormikFilter = getFilterValueFromURL(
    query,
    defaultFilters
  );
  ...
  //Zustand stores
  const [setPage, setLastPage, setFetchCount, resetRowSelection, setCursor] = useDivisionStore(
    (state) => [
      state.setPage,
      state.setLastPage,
      state.setFetchCount,
  ...
  const handleFormikSubmit = (
    values: Partial<DivisionFormikFilter>,
    formik: FormikHelpers<DivisionFormikFilter>
  ) => {
    const params = {
      ...query,
      ...(getParamsObject(values, defaultFilters) as Partial<DivisionSearchParams>),
    };
  ...
  const renderFormik = (formik: FormikProps<DivisionFormikFilter>) => {
...
const filtered = isEqual(defaultFilters, formik.values);
    ...
    const handleClearClick: MouseEventHandler = (e) => {
      e.preventDefault();
      formik.setValues(defaultFilters as DivisionFormikFilter);
formik.submitForm();
    };
