...
import { ProviderSourceColumns } from "@/components/provider-sources/ProviderSourceColumns";
import { ProviderSourceMultiCreateDeleteDialog } from "@/components/provider-sources/ProviderSourceMultiCreateDeleteDialog";
...
interface ProviderSourceFormArrayProps {
  formik: FormikProps<{ ProviderSources: ProviderSourceFormikShape[] }>;
}

const ProviderSourceFormArray: React.FC<ProviderSourceFormArrayProps> = ({ formik }) => {
  //URL States
  const { router, query, pathname } = useURL<ProviderSourceSearchParams>();
  ...
  const { setRecordsToDelete } = useProviderSourceDeleteDialog();
  ...
  const {
    ...
    isFetching,
    fetchNextPage,
  } = useInfiniteQuery<GetProviderSourcesResponse>(["providerSources"], { enabled: false });
  ...
  const dataRowCount = providerSourceData
    ? providerSourceData.pages
        .slice(0, page)
  ...
  const getCurrentData = (page: number) => {
    return [
      ...providerSourceData!.pages[page - 1].rows.map((item, index) => ({
        ...item,
        index,
        touched: false,
      })),
      {
        ...
        index: providerSourceData!.pages[page - 1].rows.length,
      },
    ];
  };
  ...
  const addRow = () => {
    formik.setFieldValue(`ProviderSources`, [
      ...formik.values.ProviderSources.map((item) => ({ ...item })),
      { ...DEFAULT_PROVIDERSOURCE },
    ]);
    setWillFocus(true);
  };
  ...
  const setTouchedRows = (idx: number) => {
    formik.setFieldValue(`ProviderSources[${idx}].touched`, true);
  };

  const deleteRow = (idx: number) => {
    const id = formik.values.ProviderSources[idx].id;

    if (id) {
      setRecordsToDelete([id.toString()]);
    } else {
      formik.setFieldValue(`ProviderSources`, [
        ...formik.values.ProviderSources.slice(0, idx),
        ...formik.values.ProviderSources.slice(idx + 1),
      ]);
      formik.setErrors({});
      resetRowSelection();
    }
  ...
  const deleteSelectedRows = () => {
    ...
    const deletedIDs = formik.values.ProviderSources.filter((_, idx) =>
    ...
    if (deletedIDs.length > 0) {
      ...
      formik.setFieldValue(
        `ProviderSources`,
        removeItemsByIndexes(formik.values.ProviderSources, indexes)
      );
      formik.setErrors({});
      resetRowSelection();
    }
  ...
  const toggleSelectAllRow = () => {
    if (Object.keys(rowSelection).length === formik.values.ProviderSources.length) {
      resetRowSelection();
    } else {
      setRowSelectionToAll(formik.values.ProviderSources.length);
    }
  };

  const goToPreviousPage = () => {
    if (providerSourceData) {
  ...
  const providerSourceTable = useReactTable<ProviderSourceFormikShape>({
    data: formik.values.ProviderSources,
    columns: ProviderSourceColumns,
    state: {
      sorting: sorting,
      rowSelection,
    },
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-4">
          <div className="text-sm">
            {providerSourceTable.getFilteredSelectedRowModel().rows.length} of{" "}
            {providerSourceTable.getFilteredRowModel().rows.length} row(s) selected.
          </div>
          {hasSelected && (
            <Button
              type="button"
        ...
        <div className="border rounded-md">
          <Table>
            <TableHeader>
              {providerSourceTable.getHeaderGroups().map((headerGroup) => (
            ...
            <TableBody>
              {providerSourceTable.getRowModel().rows?.length ? (
                providerSourceTable.getRowModel().rows.map((row) => (
                  <TableRow
                    key={row.id}
                ...
                <TableRow>
                  <TableCell
                    colSpan={ProviderSourceColumns.length}
      ...
      <ProviderSourceMultiCreateDeleteDialog />
    </Form>
  );
};

export default ProviderSourceFormArray;
