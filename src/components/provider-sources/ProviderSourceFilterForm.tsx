...
import { DEFAULT_FILTERS,CONTROL_OPTIONS, } from "@/utils/constants/ProviderSourceConstants";
...
const ProviderSourceFilterForm: React.FC = () => {
  ...
  const initialValues: ProviderSourceFormikFilter = getFilterValueFromURL(
    query,
    defaultFilters
  );
  ...
  //Zustand stores
  const [setPage, setLastPage, setFetchCount, resetRowSelection, setCursor] = useProviderSourceStore(
    (state) => [
      state.setPage,
      state.setLastPage,
      state.setFetchCount,
  ...
  const handleFormikSubmit = (
    values: Partial<ProviderSourceFormikFilter>,
    formik: FormikHelpers<ProviderSourceFormikFilter>
  ) => {
    const params = {
      ...query,
      ...(getParamsObject(values, defaultFilters) as Partial<ProviderSourceSearchParams>),
    };
  ...
  const renderFormik = (formik: FormikProps<ProviderSourceFormikFilter>) => {
...
const filtered = isEqual(defaultFilters, formik.values);
    ...
    const handleClearClick: MouseEventHandler = (e) => {
      e.preventDefault();
      formik.setValues(defaultFilters as ProviderSourceFormikFilter);
formik.submitForm();
    };
...
<FormikControl
  name="q"
  placeholder="Filter Provider Sources..."
  type="Text"
...
export default ProviderSourceFilterForm;
