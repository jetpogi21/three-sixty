...
import { createProviderType, deleteProviderTypes, updateProviderType } from "@/utils/api/ProviderTypeLibs";



const ModelObject = ProviderType;

//Generated by GeneratefindOptions
const findOptions: FindOptions<typeof ProviderType> = {
...
export const GET = async (req: Request) => {
  ...
  const findOptionsCopy: FindOptions<typeof ProviderType> = cloneDeep(findOptions);
  const providerTypeAttributes = Object.keys(ProviderType.getAttributes()).map(
    (attribute) => attribute
  );

  const searchParams = new URL(req.url).searchParams;

  const query = parseParams(searchParams) as Partial<ProviderTypeSearchParams>;
  ...
  const sort = query["sort"]
    ? providerTypeAttributes.includes(query["sort"]) ||
      providerTypeAttributes.includes(query["sort"].substring(1))
      ? query["sort"]
      : DEFAULT_SORT_BY
    : DEFAULT_SORT_BY;
...
{ name: { [Op.like]: `%${q}%` } }
    ...
    let recordCount = 0;
    if (fetchCount) {
      recordCount = await ProviderType.count({
    ...
    try {
      const data = await ProviderType.findAll(findOptionsCopy);
...
export const DELETE = async (req: Request) => {
  const body = (await req.json()) as ProviderTypeDeletePayload;
  const { deletedProviderTypes } = body;

  if (deletedProviderTypes.length > 0) {
    const t = await sequelize.transaction();
    try {
      await deleteProviderTypes(PRIMARY_KEY, deletedProviderTypes, t);
      t.commit();
      return NextResponse.json({
        status: "success",
        recordsDeleted: deletedProviderTypes.length,
      });
    } catch (error) {
      t.rollback();
      return handleSequelizeError(error);
