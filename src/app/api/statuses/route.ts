...
import { createStatus, deleteStatus, updateStatus } from "@/utils/api/StatusLibs";



const ModelObject = Status;

//Generated by GeneratefindOptions
const findOptions: FindOptions<typeof Status> = {
...
export const GET = async (req: Request) => {
  ...
  const findOptionsCopy: FindOptions<typeof Status> = cloneDeep(findOptions);
  const statusAttributes = Object.keys(Status.getAttributes()).map(
    (attribute) => attribute
  );

  const searchParams = new URL(req.url).searchParams;

  const query = parseParams(searchParams) as Partial<StatusSearchParams>;
  ...
  const sort = query["sort"]
    ? statusAttributes.includes(query["sort"]) ||
      statusAttributes.includes(query["sort"].substring(1))
      ? query["sort"]
      : DEFAULT_SORT_BY
    : DEFAULT_SORT_BY;
...
{ name: { [Op.like]: `%${q}%` } }
    ...
    let recordCount = 0;
    if (fetchCount) {
      recordCount = await Status.count({
    ...
    try {
      const data = await Status.findAll(findOptionsCopy);
...
export const DELETE = async (req: Request) => {
  const body = (await req.json()) as StatusDeletePayload;
  const { deletedStatus } = body;

  if (deletedStatus.length > 0) {
    const t = await sequelize.transaction();
    try {
      await deleteStatus(PRIMARY_KEY, deletedStatus, t);
      t.commit();
      return NextResponse.json({
        status: "success",
        recordsDeleted: deletedStatus.length,
      });
    } catch (error) {
      t.rollback();
      return handleSequelizeError(error);
